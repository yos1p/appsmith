...
const parseJSObjectWithAST = (jsObjectBody) => {
    ...
    JSObjectProperties.forEach((node) => {
        ...
        let property = {
            key: astring.generate(propertyNode.key),
            value: astring.generate(propertyNode.value),
...
const isIdentifierNode = (node) => {
    return node.type === exports.NodeTypes.Identifier;
};
const isMemberExpressionNode = (node) => {
    return node.type === exports.NodeTypes.MemberExpression;
};
const isVariableDeclarator = (node) => {
    return node.type === exports.NodeTypes.VariableDeclarator;
};
const isFunctionDeclaration = (node) => {
    return node.type === exports.NodeTypes.FunctionDeclaration;
};
const isFunctionExpression = (node) => {
    return node.type === exports.NodeTypes.FunctionExpression;
};
const isArrowFunctionExpression = (node) => {
    return node.type === exports.NodeTypes.ArrowFunctionExpression;
};
const isObjectExpression = (node) => {
    return node.type === exports.NodeTypes.ObjectExpression;
};
const isAssignmentPatternNode = (node) => {
    return node.type === exports.NodeTypes.AssignmentPattern;
};
const isLiteralNode = (node) => {
    return node.type === exports.NodeTypes.Literal;
};
const isPropertyNode = (node) => {
    return node.type === exports.NodeTypes.Property;
};
const isPropertyAFunctionNode = (node) => {
    return (node.type === exports.NodeTypes.ArrowFunctionExpression ||
        node.type === exports.NodeTypes.FunctionExpression);
};
const isArrayAccessorNode = (node) => {
    return (isMemberExpressionNode(node) &&
        node.computed &&
        isLiteralNode(node.property) &&
        lodash.isFinite(node.property.value));
};
const wrapCode = (code) => {
    return `
    (function() {
...
const getAST = lodash.memoize((code, options) => acorn.parse(code, Object.assign(Object.assign({}, options), { ecmaVersion: ECMA_VERSION })));
    ...
    try {
        ...
        */
        ...
        const referencesArr = Array.from(references).filter((reference) => {
            ...
            const topLevelIdentifier = lodash.toPath(reference)[0];
            return !(functionalParams.has(topLevelIdentifier) ||
                variableDeclarations.has(topLevelIdentifier) ||
                lodash.has(invalidIdentifiers, topLevelIdentifier));
        });
        return {
...
const getPropertyAccessor = (propertyNode) => {
    ...
    else if (isLiteralNode(propertyNode) && lodash.isString(propertyNode.value)) {
        // is string literal search a['b']
        return `.${propertyNode.value}`;
    }
    else if (isLiteralNode(propertyNode) && lodash.isFinite(propertyNode.value)) {
        // is array index search - a[9]
        return `[${propertyNode.value}]`;
    }
};
const isTypeOfFunction = (type) => {
    return (type === exports.NodeTypes.ArrowFunctionExpression ||
        type === exports.NodeTypes.FunctionExpression);
};
    ...
    acornWalk.simple(ast, {
        MemberExpression(node) {
            ...
            if (isLiteralNode(property) &&
                lodash.isString(property.value) &&
...
exports.extractInvalidTopLevelMemberExpressionsFromCode = extractInvalidTopLevelMemberExpressionsFromCode;
...
exports.parseJSObjectWithAST = parseJSObjectWithAST;
