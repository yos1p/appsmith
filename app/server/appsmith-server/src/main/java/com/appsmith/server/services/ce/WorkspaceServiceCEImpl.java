...
public class WorkspaceServiceCEImpl extends BaseService<WorkspaceRepository, Workspace, String>
     ...
     * is discarded.
     *
     * We force the creation of the workspace for the user because we are creating the default workspace for the user
     * regardless if they have the permission CREATE_WORKSPACES or not. Typically, this function is used during user signup.
     *
    ...
    /**
     * Create the workspace for the user. This is the default method that should be used to create a workspace for a user
     * We don't force the creation of the workspace and only create it if the user has permissions to do so.
     * @param workspace
     * @param user
     * @return
     */
    ...
    public Mono<Workspace> create(Workspace workspace, User user) {
        return create(workspace, user, false);
    }
     ...
     */
    @Override
    public Mono<Workspace> create(Workspace workspace, User user, Boolean forceCreateWorkspace) {
        if (workspace == null) {
        ...
        // If we are force creating the workspace, this value will always be true. Else, we check the user's permissions
        Mono<Boolean> createWorkspaceAllowedMono = forceCreateWorkspace ? Mono.just(TRUE) : isCreateWorkspaceAllowed();
